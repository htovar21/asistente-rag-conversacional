...
# Configuración del Vector Store (Misma que en la app)
embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2")
vector_store = PineconeVectorStore(index_name=CONFIG["PINECONE_INDEX_NAME"], embedding=embeddings)

USERNAME_DEFAULT = "admin" # Usuario que aparecerá como dueño

def bulk_upload():
    folder_path = "manuales" # <--- CARPETA DONDE PONES TUS PDFS
    ...
    for i, filename in enumerate(files):
        ...
        try:
            ...
            if not text:
                print(f"   ⚠️ PDF Vacío o imagen: {filename}")
                continue

            # 2. Vectorizar (Chunk 1500)
            docs, ids = process_text_to_docs(text, filename)
            
            # Borrar previo
            try: vector_store.delete(filter={"source": filename})
            except: pass
            
            vector_store.add_documents(docs, ids=ids)
            
            # 3. Subir a Supabase Storage
            storage_path = f"{USERNAME_DEFAULT}/{sanitize_filename_to_ascii(filename)}"
            ...
            supabase_admin.table('manuales').upsert({
                ...
                'uploader_username': USERNAME_DEFAULT, 
